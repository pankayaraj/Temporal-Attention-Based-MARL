...
def setup_master(args, env=None, return_env=False):
    ...
    for i, agent in enumerate(env.world.policy_agents):
        ...
        if hasattr(agent, 'adversary') and agent.adversary:
            if policy1 is None:
                policy1 = MPNN(input_size=pol_obs_dim,num_agents=num_adversary,num_entities=num_entities,hidden_dim=hidden_dim, action_space=action_space,
                               pos_index=pos_index, mask_dist=args.mask_dist,entity_mp=entity_mp).to(args.device)
            team1.append(Neo(args,policy1,(obs_dim,),(1,hidden_dim,),action_space))
        else:
            if policy2 is None:
                policy2 = MPNN(input_size=pol_obs_dim,num_agents=num_friendly,num_entities=num_entities,hidden_dim=hidden_dim, action_space=action_space,
                               pos_index=pos_index, mask_dist=args.mask_dist,entity_mp=entity_mp).to(args.device)
            team2.append(Neo(args,policy2,(obs_dim,),(hidden_dim,), action_space))
    ...
    def eval_act(self, obs, recurrent_hidden_states, mask):
        ...
        for i in range(len(obs)):
            ...
            if hasattr(agent, 'adversary') and agent.adversary:
                ...
                mask1.append(torch.as_tensor(mask[i],dtype=torch.float,device=self.device).view(1,-1))
            else:
                obs2.append(torch.as_tensor(obs[i],dtype=torch.float,device=self.device).view(1,-1))
                mask2.append(torch.as_tensor(mask[i],dtype=torch.float,device=self.device).view(1,-1))
        if len(obs1)!=0:
            all_obs.append(obs1)
        if len(obs2)!=0:
            all_obs.append(obs2)
        ...
        if len(mask2) != 0:
            all_mask.append(mask2)


        actions = []
        for team,policy,obs, mask in zip(self.teams_list,self.policies_list,all_obs, all_mask):
            if len(obs)!=0:
                _,action,_,_ = policy.act(torch.cat(obs).to(self.device),None,torch.cat(mask).to(self.device),deterministic=True)
