...
class MPNN(nn.Module):
    def __init__(self, action_space, num_agents, num_entities, input_size=16, hidden_dim=128, embed_dim=None,
        ...
        self.update = nn.Sequential(nn.Linear(self.h_dim+self.embed_dim,self.h_dim),
                                    self.nonlin(inplace=True))
        self.temporal_unit = nn.LSTM(input_size=self.h_dim, hidden_size=self.h_dim)

        self.value_head = nn.Sequential(nn.Linear(2*self.h_dim, self.h_dim),
                                        self.nonlin(inplace=True),
                                        nn.Linear(self.h_dim,1))

        self.policy_head = nn.Sequential(nn.Linear(2*self.h_dim, self.h_dim),
                                         self.nonlin(inplace=True))
    ...
    def _fwd(self, inp, state, mask):
        # inp should be (batch_size,input_size)
        # inp - {iden, vel(2), pos(2), entities(...)}
        agent_inp = inp[:,:self.input_size]

        batch_size = inp.shape[0]
        if self.prev_batch_size == None:
            self.prev_batch_size == batch_size

        if batch_size != self.prev_batch_size:
            self.prev_hidden = [torch.rand(1, batch_size, self.h_dim).to(self.device),
                      torch.rand(1, batch_size, self.h_dim).to(self.device)]

        hidden_init = [torch.rand(1, batch_size, self.h_dim).to(self.device),
                      torch.rand(1, batch_size, self.h_dim).to(self.device)]

        hidden = [None, None]
        hidden[0]  = mask*self.prev_hidden[0] + (1-mask)*hidden_init[0]
        hidden[1]  = mask*self.prev_hidden[1] + (1-mask)*hidden_init[1]
        ...
        h_new = h.clone().view(1, -1, self.h_dim)
        #print(state.shape, h.shape)
        #print(batch_size)
        h_new, self.prev_hidden = self.temporal_unit(h_new, hidden)
        h_new = h_new.view(batch_size, self.h_dim)

        h = torch.cat([h, h_new], dim=1)

        return h, state # should be <batch_size, self.h_dim> again
...
class LSTM_FOR_GRAPH(nn.Module):
    ...
    def __init__(self,
        ...
        super(LSTM_FOR_GRAPH, self).__init__()
